...
def blendDemo():
    im1 = cv2.cvtColor(cv2.imread('input/brooklyn_bridge.jpg'), cv2.COLOR_BGR2RGB) / 255
    im2 = cv2.cvtColor(cv2.imread('input/man.jpg'), cv2.COLOR_BGR2RGB) / 255
    mask = cv2.cvtColor(cv2.imread('input/man_mask.jpeg'), cv2.COLOR_BGR2RGB) / 255
    ...
    cv2.imwrite('man_on_brooklean.png', cv2.cvtColor((im_blend * 255).astype(np.uint8), cv2.COLOR_RGB2BGR))
